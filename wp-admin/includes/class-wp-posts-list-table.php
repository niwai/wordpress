...
/* 階層アイコンカスタマイズ（2017/06/27 堀越）start */
//		$pad = str_repeat( '&#8212; ', $this->current_level );
		...
		if ($this->current_level <= 1) {
			// 最上位階層はフォルダアイコンのみを表示
			$icon = '<img src="../wp-content/plugins/media-file-manager/images/dir.png" width="20" height="18"/> ';
			if ($this->current_level == 1) {
				// 第1階層はインデントを付ける
				$icon = '┗ '.$icon;
			}
		} else {
			$icon = str_repeat( '　 ', $this->current_level).str_repeat( '┗ ', $this->current_level - 1 );
		}
		...
		if ( $can_edit_post && $post->post_status != 'trash' ) {
			printf(
				$icon.'<a class="row-title" href="%s" aria-label="%s">%s%s</a>',
		...
		_post_states( $post );
/* 階層アイコンカスタマイズ（2017/06/27 堀越）end */
