...
<?php
  /* コンテンツヘッダー */
  echo apply_filters('the_content', get_page_by_path('fair/'.CODE.'/'.$status.'hdr', OBJECT, 'page') -> post_content);
?>
...
<?php
  if ($status == '') {
    /* サイドバー */
    echo apply_filters('the_content', get_page_by_path('fair/'.CODE.'/side', OBJECT, 'page') -> post_content);
  }
  /* メインコンテンツ */
  echo apply_filters('the_content', get_page_by_path('fair/'.CODE.'/'.$status, OBJECT, 'page') -> post_content);
?>
</div><!-- main END-->
